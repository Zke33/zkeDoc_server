...
func (RoleDocApi) RoleDocUpdateView(c *gin.Context) {
	...
	// 前端传递来的id列表
	// 后端本来就有的id列表
	var set1 = make([]uint, 0)
	var set2 = make([]uint, 0)
	var docMap = make(map[uint]DocItem)
	for _, s := range cr.DocList {
		set1 = append(set1, s.DocID)
		docMap[s.DocID] = s
	}
	global.DB.Model(models.RoleDocModel{}).Where("role_id = ?", cr.RoleID).Select("doc_id").Scan(&set2)
	...
	if len(addSet) > 0 {
		...
		if err := global.DB.Create(&roleDocList).Error; err != nil {
